...
function doImages() {
  // Get the actual <img> elements inside the div containers
  ...
  // ------------  this part under here needs working on, otherwise sort of works
  else {
    ...
    // growing totalHeight here : important
    if (lowerPageNum > upperPageNum + 1) { 
        for (let i = upperPageNum + 1; i < lowerPageNum; i++) {
            const imgDiv = document.getElementById(`page${i}`);
            console.log(imgDiv);
    ...
    // should be good here
    //console.log("total height initially is : " +totalHeight);
    ...
    if (totalHeight > fullWidth) {
          ...
          totalHeight = totalHeight * scaleFactor; 
          totalWidth =  totalWidth  * scaleFactor;
          //console.log(`Scaling by: ${scaleFactor}, New width: ${totalWidth}, New height: ${totalHeight}`);
    }     
    ...
    // Draw intermediate images if they exist between upperPageNum and lowerPageNum
    if (lowerPageNum > upperPageNum + 1) {
        for (let i = upperPageNum + 1; i < lowerPageNum; i++) {
            const imgDiv = document.getElementById(`page${i}`);
            const imgElement = imgDiv.querySelector('img');

            // Draw full height of the image with scaling
            ctx.drawImage(imgElement, 0, 0, imgElement.width, imgElement.height, 0, currentYPosition, imgElement.width * scaleFactor, imgElement.height * scaleFactor);
            currentYPosition += imgElement.height * scaleFactor; // Adjust currentYPosition based on scaleFactor
        }
    }
  ...
  console.log(`Rotated canvas values , Height : ${rotatedCanvas.height} and width : ${rotatedCanvas.width}`);
  ...
  let centerX = (fullWidth - rotatedCanvas.width) / 2; //assuming the manhwa is centered
  console.log(`So we are doing this : ${fullWidth}  - ${rotatedCanvas.width} and divide result by 2 to equal : ${centerX}`)

  rotatedCanvas.style.left = `${centerX}px`;
  
  let positionOffset = selection.lower.y;

  rotatedCanvas.style.top = `${selection.lower.wrapDiv.offsetTop + positionOffset - (rotatedCanvas.height/2)}px`;   // Align it based on where the lower click is 
...
function start() {
    ...
    function selectRegion(event) {
        const wrapDiv = event.target.closest('.wrap_img'); // Get the div element
        const img = wrapDiv.querySelector('img'); // Get the image inside the div
        const rect = img.getBoundingClientRect();
        const imgId = wrapDiv.id; // The div's id
        const imageHeight = img.naturalHeight;

        // Extract the numeric page number from the image IDs (e.g., "page10" => 10)
        const imgNumber =    parseInt(imgId.replace("page", ""));

        const yOffset = event.clientY - rect.top;  // Get the click position relative to the div

        // console.log(`Clicked on ${imgId}, yOffset: ${yOffset}, image height: ${imageHeight}`);

        if (!selection.upper) {
            // First click: set upper bound
            selection.upper = { image:img, pageNumber: imgNumber, y: yOffset, height: imageHeight , wrapDiv : wrapDiv };

            //console.log(`Upper bound set at ${selection.upper.y} on page ${selection.upper.pageNumber}`);
        } else
        if (!selection.lower) {
            // Second click: set lower bound
            selection.lower = { image:img, pageNumber: imgNumber, y: yOffset, height: imageHeight , wrapDiv : wrapDiv };
            //console.log(`Lower bound set at ${selection.lower.y} on page ${selection.lower.pageNumber}`);
    ...
    function initSelection() {
        const images = document.querySelectorAll('.wrap_img img');
        
        images.forEach(img => {
            img.removeEventListener('click', selectRegion ); // to help with errors on multiple clicks 
        });
...
// just copy and paste this in console
// this is as good as it gets, think thats complete